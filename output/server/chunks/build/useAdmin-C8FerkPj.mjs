...
var useAdmin = () => {
	...
	const muatStatistikPeserta = async () => {
		...
		try {
			...
			if (data) {
				...
				totalHadir.value = data.filter((p) => p.is_scanned).length;
			}
		} catch (err) {
	...
	const muatDaftarPeserta = async (query = "", status = "semua", hadir = "semua", limit = 50) => {
		...
		try {
			...
			if (hadir === "hadir") req = req.eq("is_scanned", true);
			else if (hadir === "belum") req = req.eq("is_scanned", false);
...
//# sourceMappingURL=useAdmin-C8FerkPj.mjs.map
